...
void configuresound(){
	#ifdef EXCESSIVE
	printf("configuresound\n");
	#endif
	if (optionselected>4)optionselected=0;
	if (optionselected<0)optionselected=4;
	static char optionv[4][35];
	static char options[5][35];
	sprintf(options[0], "Drum Volume");
	sprintf(options[1], "Guitar Volume");
	sprintf(options[2], "Guitar 2 Volume");
	sprintf(options[3], "BGM Volume");
	sprintf(options[4], "Return to Options");
	sprintf(optionv[0], "%i", voldrum);
	sprintf(optionv[1], "%i", volguit);
	sprintf(optionv[2], "%i", volbass);
	sprintf(optionv[3], "%i", volbgm);
	static drawtext optionsd[5];
	static drawtext optionvd[4];
	for (int i=0;i<5;i++){
		...
		if (optionselected==i&&suboptionselection==true){
			...
			if(i<4)optionvd[i].draw(uivars.options_table_x+350,uivars.options_table_y-(uivars.options_table_s*i),i*10,255,255,255,255,false,true);}
	}
}
