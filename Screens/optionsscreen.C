...
void optionsscreen(int totalframes){
	...
	char optionv[10][256];
	if (optionselected<0)optionselected=16;
	if (optionselected>16)optionselected=0;
	static char options[17][256];
	...
	sprintf(options[9],"Verticle Sync");
	sprintf(options[10],"Noteskin");
	sprintf(options[11],"Configure Drum Controls...");
	sprintf(options[12],"Configure Guitar 1 Controls...");
	sprintf(options[13],"Configure Guitar 2 Controls...");
	sprintf(options[14],"Configure Sound Settings...");
	sprintf(options[15],"Configure Arcade Settings...");
	sprintf(options[16],"Back");
	static drawtext optionsd[17];
	static drawtext optionvd[11];
	for (int i=0;i<17;i++){
	...
	if (!screenfaddingin&&!screenfaddingout){
		...
		if (verticlesync){sprintf(optionv[9],"true");}
		else{sprintf(optionv[9],"false");}
