...
void sort_abc(){
	...
	if (curcatcount>0){
		...
		for (int a=0;a<curcatcount;a++){
			...
			snprintf(curcatlist[a],255,"%c", 65+a);
		}
		snprintf(curcatlist[curcatcount-1],255,"other");
	...
	for (int a=0;a<gbsongcount;a++){
		if (drummode&&!guitar1mode&&!guitar2mode){
			if(songs[a].drumbasdiff>0||songs[a].drumadvdiff>0||songs[a].drumextdiff>0){
				...
				for (int b=0;b<curcatcount-1;b++){
					if (songs[a].name[0]==curcatlist[b][0]||songs[a].name[0]==curcatlist[b][0]+32){
						curcatsongcount[b]++;
						cursongcat[n]=b;	
						addanother=false;
					}				
				...
				if (addanother){
					cursongcat[n]=curcatcount-1;
					curcatsongcount[curcatcount-1]++;
				}
				cursongnum[n]=a;
